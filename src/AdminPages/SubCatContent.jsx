...
export default function SubCatContent() {
    const [subcategories, setSubcategories] = useState([
        { parent: 'Electronics', sub: 'Smartphones' },
        { parent: 'Electronics', sub: 'Laptops' },
        { parent: 'Fashion', sub: "Men's Clothing" },
        { parent: 'Fashion', sub: "Women's Clothing" }
    ])

    const [showModal, setShowModal] = useState(false)
    const [newSubcategory, setNewSubcategory] = useState({ parent: 'Electronics', sub: '' })
    const { isDarkMode } = useTheme()

    const categories = ['Electronics', 'Fashion', 'Home & Garden', 'Sports']

    const addSubcategory = () => {
        if (newSubcategory.sub.trim()) {
            setSubcategories([...subcategories, { ...newSubcategory, sub: newSubcategory.sub.trim() }])
                    ...
                    {subcategories.map((item, index) => (
                        ...
                        >
                            <div className="flex items-center space-x-3">
                                <FolderTree className="h-5 w-5 text-[#155dfd]" />
                                <div>
                                    <span className={`font-bold text-lg ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>{item.sub}</span>
                                    <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Parent: {item.parent}</p>
                                </div>
                            </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className={`rounded-2xl p-8 w-full max-w-md shadow-2xl border ${isDarkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-200'}`}>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className={`block text-sm font-semibold mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Parent Category</label>
                                <select
                            ...
                            <div>
                                <label className={`block text-sm font-semibold mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Subcategory Name</label>
                                <input
                                    type="text"
                                    value={newSubcategory.sub}
                                    onChange={(e) => setNewSubcategory({ ...newSubcategory, sub: e.target.value })}
